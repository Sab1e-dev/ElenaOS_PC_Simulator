...
function test_lvgl_functions() {
  try {
    ...
    lv_obj_set_size(base_obj, 700, 380);
    lv_obj_align(base_obj, LV_ALIGN_TOP_MID, 0, 20);
...
function test_label_functions(scr) {
  ...
  // 设置字体
  lv_obj_set_style_text_font(label, lv_font.lv_font_montserrat_20, 0);
...
function test_button_functions(scr) {
  ...
  //// 测试按钮状态
  lv_obj_set_state(btn, LV_STATE_PRESSED, LV_STATE_CHECKED);
...
function test_dropdown_functions(scr) {
  ...
  lv_obj_align(dropdown, LV_ALIGN_CENTER, 0, 150);

  // 设置选项
  lv_dropdown_set_options(dropdown, "Option 1\nOption 2\nOption 3");

  // 设置选中项
  lv_dropdown_set_selected(dropdown, 1, LV_ANIM_ON);
...
function test_textarea_functions(scr) {
  ...
  lv_obj_align(ta, LV_ALIGN_CENTER, 0, 300);
...
function test_checkbox_functions(scr) {
  ...
  lv_obj_align(cb, LV_ALIGN_CENTER, -100, -150);
  ...
  //// 测试选中状态
  print("Checkbox checked:", lv_obj_has_state(cb, LV_STATE_CHECKED));
...
function test_arc_functions(scr) {
  ...
  lv_obj_align(arc, LV_ALIGN_CENTER, 100, -150);
...
function test_chart_functions(scr) {
  ...
  lv_obj_align(chart, LV_ALIGN_CENTER, -150, 150);
...
function test_table_functions(scr) {
  ...
  lv_obj_align(table, LV_ALIGN_CENTER, 150, 150);
...
function test_roller_functions(scr) {
  ...
  lv_obj_align(roller, LV_ALIGN_CENTER, 150, -50);
...
function create_msgbox(scr) {
  // 创建消息框
  let mbox = lv_msgbox_create(scr);
  lv_obj_set_size(mbox, 300, 200);

  lv_msgbox_add_title(mbox, "Setting");
  lv_msgbox_add_text(mbox, "This is a message box.");
  let minimize_button = lv_msgbox_add_header_button(mbox, LV_SYMBOL_MINUS);

  register_lv_event_handler(
    minimize_button,
    LV_EVENT_CLICKED,
    function (e) {
      let mbox = lv_event_get_user_data(e);
      lv_obj_add_flag(mbox, LV_OBJ_FLAG_HIDDEN);
      print("Message box minimized.");
    },
    mbox
  )
  ...
  let btn = lv_msgbox_add_footer_button(mbox, "Apply");
...
function test_msgbox_functions(scr) {
  ...
  let btn_label = lv_label_create(btn);
  lv_obj_center(btn_label);
  lv_label_set_text(btn_label, "Show Message Box");
  register_lv_event_handler(
    btn,
    LV_EVENT_CLICKED,
    function (e) {
...
function test_style_functions(scr) {
  ...
  lv_obj_align(obj, LV_ALIGN_CENTER, -150, -50);
...
function test_event_functions(scr) {
  ...
  lv_obj_align(btn, LV_ALIGN_DEFAULT, 50, 200);

  let btn_label = lv_label_create(btn);
  lv_label_set_text(btn_label, "Test Event");
  lv_obj_center(btn_label);
